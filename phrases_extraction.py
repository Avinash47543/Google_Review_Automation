    ...
    try:
        ...
        if "result" in response_data:
            ...
            for line in result_text.split('\n'):
                ...
                # Handle both quoted and unquoted phrases
                if line.startswith('"') and line.endswith('"'):
                    phrase = line[1:-1]
                    sentiment = "positive" if sentiment.lower() == "positive" else "negative"
                elif '(' in line and ')' in line:
                    parts = line.split('(')
                    phrase = parts[0].strip().strip('"')
                    sentiment = parts[1].split(')')[0].strip().lower()
                else:
                    # Fallback - use review sentiment
                    phrase = line.strip('"')
                    sentiment = sentiment.lower()
                
                if phrase:  # Only add if we have a phrase
                    phrases.append({
                        'Phrase': phrase,
                        'Sentiment': sentiment
                    })
...
def process_phrases(classified_file, phrase_output):
    try:
        ...
        required_columns = ['xid', 'Project name', 'Review', 'Sentiment']
        ...
        with open(phrase_output, 'w', newline='', encoding='utf-8') as csvfile:
            ...
            for _, row in df.iterrows():
                ...
                if not review:  # Skip empty reviews
                    continue
                    
                sentiment = str(row['Sentiment']).strip().lower()
                if sentiment not in ['positive', 'negative']:
                    continue  # Skip if sentiment isn't valid
                ...
                csvfile.flush()  # Ensure data is written after each review
